...
fetch('https://mh-amazing.herokuapp.com/amazing')
  ...
  .then( res => {
    ...
    pastEvents = eventos.filter(e => e.date < fechaActual)
    crearCheckbox(pastEvents, boxes)
    imprimirCard(pastEvents, containerCards)
...
function filtrar(){
  ...
  let filtradoPorCategoria = pastEvents.filter(evento => checked.includes(evento.category) || checked.length === 0)
